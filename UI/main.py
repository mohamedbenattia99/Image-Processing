...
class main_app:
    def __init__(self, window):
        self.window = window
        self.window.title('Projet Traitement d''images - Réalisé par MBA,MKO,WS GL4')
        self.window.geometry(f'{self.window.winfo_screenwidth() }x{self.window.winfo_screenheight() }+1+0')
    ...
    def init_interface(self):
        ...
        image_menu.add_command(label="Histogramme Cumulative")
        ...
        tk.Label(file_frame, fg="green", text="Importer une image").grid(row=0, column=0, columnspan=2, padx=2)
        tk.Label(file_frame, text="Fichier").grid(row=1, column=0, padx=2)
        self.file_name = tk.Entry(file_frame, width=30)
        self.file_name.grid(row=1, column=1, padx=10)
        tk.Button(file_frame, text="Browse", padx=10, pady=5, command=self.open_callback).grid(row=1, column=2, padx=10)
        ...
        plot_widget.config(width=self.window.winfo_screenwidth() * 0.15, height=200)
        ...
        tk.Label(properties_frame, fg='green', text="Proprietés:").grid(row=0, column=0, columnspan=3, padx=2,
                                                                      pady=5)

        tk.Label(properties_frame, text="Nombre de pixel:").grid(row=3, column=0, padx=10, pady=5, sticky="w")
        self.pixel_text = tk.Label(properties_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        self.width_text = tk.Label(properties_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        self.height_text = tk.Label(properties_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        tk.Label(stat_frame, fg='green', text="Image statistiques:").grid(row=0, column=0, columnspan=3, padx=2, pady=5)

        tk.Label(stat_frame, text="Mean:").grid(row=1, column=0, padx=10, pady=5, sticky='w')
        self.mean_text = tk.Label(stat_frame, text="0", width=20, relief=tk.SUNKEN)
        self.mean_text.grid(row=1, column=2, sticky='w')

        tk.Label(stat_frame, text="Variance:").grid(row=2, column=0, padx=10, pady=5, sticky='w')
        self.variance_text = tk.Label(stat_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        self.deviation_text = tk.Label(stat_frame, text="0", width=20, relief=tk.SUNKEN)
        self.deviation_text.grid(row=3, column=2, sticky='w')

        tk.Label(stat_frame, text="Entropie:").grid(row=4, column=0, padx=10, pady=5, sticky='w')
        self.entropy_text = tk.Label(stat_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        tk.Label(histogram_frame, text="Générer l'histograame et l'histogramme cumulé:").grid(row=0, column=0)
        self.generate_hists_button = tk.Button(histogram_frame, text="Générer", padx=10, pady=5,
                                               command=self.generate_hists_callback, state=tk.DISABLED)
        ...
        self.contrast_button = tk.Button(contrast_function_frame, text="Contrast Function", padx=10, pady=5,
                                         command=self.contrast_function_callback, state=tk.DISABLED)
        ...
        tk.Label(buttons_frame, text="Vous pouvez choisir taille du filtre").grid(row=1, column=1, pady=5, padx=5, sticky='w')
        ...
        tk.Label(new_stat_frame, fg='green', text="Propriètés de l'image modifiée").grid(row=0, column=0, columnspan=3, padx=2, pady=5)

        tk.Label(new_stat_frame, text="Mean:").grid(row=1, column=0, padx=10, pady=5, sticky='w')
        self.new_mean_text = tk.Label(new_stat_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        self.new_variance_text = tk.Label(new_stat_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        self.new_deviation_text = tk.Label(new_stat_frame, text="0", width=20, relief=tk.SUNKEN)
        self.new_deviation_text.grid(row=3, column=2, sticky='w')

        tk.Label(new_stat_frame, text="Entropie:").grid(row=4, column=0, padx=10, pady=5, sticky='w')
        self.new_entropy_text = tk.Label(new_stat_frame, text="0", width=20, relief=tk.SUNKEN)
        ...
        self.save_button = tk.Button(file_frame, text="Télécharger Nouvelle Image", padx=10, pady=5, command=self.save_callback,
                                     state=tk.DISABLED)
        ...
        tk.Label(new_image_frame, text="Image modifiée", fg='green').pack(padx=10)
        ...
        plot_widget2.config(width=self.window.winfo_screenwidth() * 0.55, height=600)
        plot_widget2.pack(expand=tk.YES, anchor=tk.CENTER, padx=5)

        tk.Label(new_stat_frame, text="CONSOLE").grid(row=3, column=0, padx=10, pady=5, sticky='w')
        console_frame = tk.Frame(new_image_frame, height=100, width=self.window.winfo_screenwidth() * 0.55, pady=10)
        self.console = tk.Text(console_frame, height=20, width=100, fg="white", state=tk.DISABLED, bg="blue")
        self.console.pack()
    ...
    def open_callback(self):
        try:
            ...
            self.write_console(f'Nouvelle image.\n({file})')
    ...
    def save_callback(self):
        try:
            ...
            self.update_orig_stats()
            self.write_console(f'Image sauvegardée !\n(path: {file})')
        except Exception:
    ...
    def equalize_callback(self):
        ...
        self.write_console("Histogram egalisé")
        self.update_new_image()
    ...
    def contrast_function_callback(self):
        ...
        if (ct.check_points(a_point, b_point) == False):
            ...
            self.new_image.set_attributes(width, height, gray_level, final_image)
            self.write_console("Contrast a été appliqué avec succès")
            self.update_new_image()
    ...
    def add_noise(self):
        ...
        self.write_console("Bruit ajouté avec succès")
        self.update_new_image()
    ...
    def average_filter(self):
        ...
        if (size == ""):
            ...
            self.new_image.set_attributes(width, height, gray_level, final_image)
            self.write_console("Filtre moyen ajouté avec succès")
            self.update_new_image()
    ...
    def median_filter(self):
        ...
        if (size == ""):
            ...
            self.new_image.set_attributes(width, height, gray_level, final_image)
            self.write_console("Filtre mediane ajouté avec succès")
            self.update_new_image()
            self.new_snr_text.config(text=str(filt.SNR(self.orig_image.get_data(), self.new_image.get_data())))
    ...
    def gauss_filter(self):
        ...
        if (size == ""):
            ...
            self.new_image.set_attributes(width, height, gray_level, final_image)
            self.write_console("Filtre gaussien ajouté avec succès")
            self.update_new_image()
            self.new_snr_text.config(text=str(filt.SNR(self.orig_image.get_data(), self.new_image.get_data())))
    ...
    def high_filter(self):
        ...
        self.new_image.set_attributes(width, height, gray_level, final_image)
        self.write_console("Filtre haut ajouté avec succès")
        self.update_new_image()
        self.new_snr_text.config(text=str(filt.SNR(self.orig_image.get_data(), self.new_image.get_data())))
